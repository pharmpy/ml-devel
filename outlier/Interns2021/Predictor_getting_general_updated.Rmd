...
```{r}
my_path <- "B:/home/USER/alzha120/Training/Datasets_Summer_Interns_2021/Karlsson-1993-Prazosin" #Change the wd
setwd(my_path) 

my_file <- "run4.mod"               #Change the run
...
cov_table <- data.frame('HT' = model$dataset$HT, 
                        'AGE' = model$dataset$AGE)    # change covariate names
cov_table <- distinct(cov_table)                # to keep the first row only for each
cov_table_alt <- cov_table                      # Variable to store calculated values
ID <- unique(model$dataset$ID)

for (n in 1:ncol(cov_table)){
...
CWRES_all <- CWRES_all[CWRES_all$CWRES != 0, ] # filter CWRES == 0
CWRES_each_max <- NULL
...
}      ## predictors are in CWRES_each_max & CWRES_each_median
