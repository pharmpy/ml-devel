...
num_folds = 10     
sig_value = 2.5  # significance value for influential individual
...
for train, test in StratifiedkFold.split(inputs1, targets):
    ...
    # Fit data to model
    history = model.fit(inputs1[train], targets[train], validation_data=(inputs1[test], targets[test]), epochs=200, verbose=0)
    #plot_history(history, 'ANN1')
            
    #to store values for plotting global predicted vs. true residual values
    test_predictions = model.predict(inputs1[test]).flatten()
